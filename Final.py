...
screen_width, screen_height = 533, 800        # 이건 벚꽃 이펙트 때 사용
pygame.display.set_caption('벚꽃 보러 갈래?')
#메인 배경화면
background = pygame.image.load('cherry_blossom.jpg')   #load된 이미지는 파이게임의 Surface(객체) 로 반환
...
class CherryPetals:
    def __init__(self):
        ...
        self.speed = random.uniform(0.2, 0.3)  # 떨어지는 속도 랜덤하게 설정
        self.rotation = random.randint(0, 15)  # 벚꽃잎 회전하도록
        self.rotation_speed = random.uniform(0.1, 0.2)  # 회전 속도

    def update(self):
        self.y += self.speed      #현재 스피드 만큼 y값 더함 = 아래로 떨어진다.
        self.rotation = (self.rotation + self.rotation_speed) % 360    #떨어지는 동안 회전 (360넘지 않게 세팅하라는데 별 차이 못느낌)

        if self.y > screen_height:     # 화면을 벗어난 벚꽃잎 다시 사용
            self.x = random.randint(0, screen_width)
    ...
    def draw(self, screen):
        rotated_image = pygame.transform.rotate(cherry_blossom_img, self.rotation)    #회전하는 이미지 그림
        screen.blit(rotated_image, (self.x, self.y))   #이미지복사 : screen.blit(복사할 이미지, 복사할 대상)

# 벚꽃잎 객체 리스트 생성 (여러 잎 동시에 그리고 관리)
petals = []
...
def diff_check_2024():
    ...
    plt.figure(figsize=(10, 6))
    plt.ylim(end_date, start_date)  # y축 범위 설정 (오름차순으로)
    plt.axhspan(int(actual_flowering_dates[0].split('-')[1]), int(actual_flowering_dates[1].split('-')[1]),
                color='magenta', alpha=0.2, label='실제 개화 시기')
